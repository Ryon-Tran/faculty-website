...
export default function Activities() {
  return (
    <div className="flex flex-col items-start w-full font-['Inter']">
      ...
      <div className="flex h-52 sm:h-[280px] items-center justify-center relative w-full">
        ...
        <p className="font-extrabold text-[28px] sm:text-[48px] text-center text-white uppercase relative">Hoạt động sinh viên</p>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-start px-4 sm:px-6 py-10 sm:py-16 w-full lg:px-6">
        <div className="flex flex-1 flex-col gap-[32px] items-start w-full">
          <div className="flex items-center w-full">
            <h2 className="font-bold text-[#c8102e] text-[24px] uppercase m-0">Tất cả hoạt động</h2>
          </div>
          <div className="flex flex-wrap gap-3 mb-2">
            {["Tất cả", "Đoàn - Hội SV", "Học bổng", "CLB", "Cựu sinh viên"].map((cat, idx) => (
              <button key={idx} className={`px-[16px] py-[8px] rounded-[20px] text-[13px] font-medium border cursor-pointer transition-colors ${idx === 0 ? 'bg-[#1a428a] text-white border-[#1a428a]' : 'bg-white text-[#666] border-[#e5e7eb] hover:border-[#1a428a] hover:text-[#1a428a]'}`}>
                {cat}
              </button>
            ))}
          </div>
          ...
          <div className="flex items-center justify-center gap-[8px] w-full mt-[24px]">
            {[1, 2, 3].map(p => (
              <button key={p} className={`size-[36px] flex items-center justify-center rounded-[6px] text-[14px] font-medium cursor-pointer border transition-colors ${p === 1 ? 'bg-[#1a428a] text-white border-[#1a428a]' : 'bg-white text-[#666] border-[#e5e7eb] hover:border-[#1a428a]'}`}>
                {p}
              </button>
            ))}
          </div>
